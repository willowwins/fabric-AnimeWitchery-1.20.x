...
public class ModLootTableModifiers {
    ...
    private static final Identifier GL_BAT_ID = new Identifier("geckolib", "entities/bat");
        ...
        LootTableEvents.MODIFY.register((resourceManager, lootManager, identifier, tableBuilder, lootTableSource) -> {
            ...
            if (identifier.getPath().contains("chests/")) {
                LootPool.Builder poolBuilder = LootPool.builder()
                        ...
                        .with(ItemEntry.builder(ModItems.STARDUST))
                        .apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(1.0f, 2.0f)).build());

                tableBuilder.pool(poolBuilder.build());
            }

            if (identifier.getPath().contains("entities/")) {
                LootPool.Builder poolBuilder = LootPool.builder()
                        .rolls(ConstantLootNumberProvider.create(1))
                        .conditionally(RandomChanceLootCondition.builder(0.1f)) // 10% chance
                        .conditionally(KilledByPlayerLootCondition.builder()) // Player kills only
                        .with(ItemEntry.builder(ModItems.STARDUST))
                        .apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(1.0f, 2.0f)).build());
