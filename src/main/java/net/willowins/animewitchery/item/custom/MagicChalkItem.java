...
public class MagicChalkItem extends Item {

    public MagicChalkItem(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            ...
            if (canDrawCircle(world, circlePos, face)) {
                System.out.println("Magic Chalk: Creating barrier circle at " + circlePos);
                // Stage 1: Create the barrier circle block entity
                createBarrierCircle(world, circlePos);
                
                // Play drawing sound
                world.playSound(null, circlePos, SoundEvents.BLOCK_BEACON_ACTIVATE, SoundCategory.BLOCKS, 0.5f, 1.2f);
                
                return TypedActionResult.success(stack);
            } else {
                System.out.println("Magic Chalk: Cannot draw circle at " + circlePos + " with face " + face);
            }
        }
    ...
    private boolean canDrawCircle(World world, BlockPos pos, Direction face) {
        ...
        if (face != Direction.UP && face != Direction.DOWN) {
            System.out.println("Magic Chalk: Not a horizontal surface - face: " + face);
            return false;
        }
        ...
        if (!canPlace) {
            System.out.println("Magic Chalk: Cannot place here - block: " + state.getBlock());
        }
